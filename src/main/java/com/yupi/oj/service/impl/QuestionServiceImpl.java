...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
    ...
    public void validQuestion(Question question, boolean add) {
        ...
        if (StringUtils.isBlank(content) && content.length()>8129){
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"内容过长");
        }
        if (StringUtils.isBlank(answer) && answer.length()>8129){
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"答案过长");
        }
        if (StringUtils.isBlank(judgeCase) && judgeCase.length()>8129){
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"判题用例过长");
        }
        if (StringUtils.isBlank(judgeConfig) && judgeConfig.length()>8129){
            throw new BusinessException(ErrorCode.PARAMS_ERROR,"判题配置过长");
        }
    }

    /**
     * 根据前端请求对象，得到 mybatis框架支持的QueryWrapper类
     * 获取查询包装类
     *
